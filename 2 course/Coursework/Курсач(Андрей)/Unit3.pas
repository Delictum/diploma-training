...
procedure TFormBegin.CBLevelChange(Sender: TObject);  //Открывает список жанров
begin
  if CBLevel.ItemIndex > 1 then
    ShowMessage('Уровень в разработке.');
...
begin
  ...
  GameStepIDName := 1;                            //Счетчик названия песни = 1
end;
...
begin
  if (DBLCBGenre.Text <> '') and (CBLevel.ItemIndex < 2) then
    begin
      if (GameStepIDBoolean <> true) and (DBLCBGenre.Text = 'Рок') and (CBLevel.ItemIndex = 0) then
        begin
          GameStepID := 1;
          GameStepIDPlay := 1;
        end;
      if (GameStepIDBoolean <> true) and (DBLCBGenre.Text = 'Классическая музыка') then
        begin
          GameStepID := 11;
          GameStepIDPlay := 11;
        end;
      if (GameStepIDBoolean <> true) and (DBLCBGenre.Text = 'Фолк') then
        begin
          GameStepID := 21;
          GameStepIDPlay := 21;
        end;
      if (GameStepIDBoolean <> true) and (DBLCBGenre.Text = 'Рок') and (CBLevel.ItemIndex = 1) then
        begin
          GameStepID := 31;
          GameStepIDPlay := 31;
        end;
      if (GameStepIDBoolean <> true) and (DBLCBGenre.Text = 'Хип-Хоп') and (CBLevel.ItemIndex = 1) then
        begin
          GameStepID := 41;
          GameStepIDPlay := 41;
        end;
      if (GameStepIDBoolean <> true) and (DBLCBGenre.Text = 'Электронная музыка') and (CBLevel.ItemIndex = 1) then
        begin
          GameStepID := 51;
          GameStepIDPlay := 51;
        end;
