...
uses Unit3, Unit1, Unit5, Unit2, Unit8;

procedure TFormGame.BtnRandomCheckBoxClick(Sender: TObject);  //Рандомные названия и расположения
var
  bufIDRandom : string;
  I, J, X, Y : Integer;
  CheckBox : TCheckBox;
begin
  ...
  repeat
    ...
    inc(I);
  until I > 4;          //Рандомное расположение
   ...
   SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
     SQLQuery1.SQL.Add('SELECT Music FROM Fake_Music WHERE ID = ' + bufIDRandom);
      SQLQuery1.Open;
       ClientDataSet1.Active := true;
  CheckBox2.Caption := VarToStr(SQLQuery1.FieldValues['Music']);           //Рандомные названия
   ...
   SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
     SQLQuery1.SQL.Add('SELECT Music FROM Fake_Music WHERE ID = ' + bufIDRandom);
      SQLQuery1.Open;
       ClientDataSet1.Active := true;
  CheckBox3.Caption := VarToStr(SQLQuery1.FieldValues['Music']);           //Рандомные названия
   ...
   SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
     SQLQuery1.SQL.Add('SELECT Music FROM Fake_Music WHERE ID = ' + bufIDRandom);
      SQLQuery1.Open;
       ClientDataSet1.Active := true;
  CheckBox4.Caption := VarToStr(SQLQuery1.FieldValues['Music']);           //Рандомные названия
   ...
   SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
     SQLQuery1.SQL.Add('SELECT * FROM Music WHERE ID = ' + IntToStr(GameStepIDPlay));
      SQLQuery1.Open;
       ClientDataSet1.Active := true;
  CheckBox1.Caption := VarToStr(SQLQuery1.FieldValues['Name_Song']);       //Правильное название
...
begin
  ...
  LabelStep.Caption :=  IntToStr(GameNumberStep) + ' из 10';
end;
...
begin
    ...
    begin
      ...
      if GameEdit.Text = VarToStr(SQLQuery1.FieldValues['Name_Song']) then
        begin
          ...
          LabelScore.Caption := 'Очки: ' + IntToStr(GameScore);
        end;
      GameEdit.Text := '';
    end
  else
  ...
  if GameNumberStep = 11 then
    begin
      ...
      FormEnd.LabelNewScore.Caption := 'Вы набрали ' + IntToStr(GameScore) + ' очков!';
      ...
      if IntToStr(GameScore) > VarToStr(FormEnd.SQLQuery1.FieldValues['Score']) then
        begin
          ShowMessage('Вы побили свой рекорд!');
          FormEnd.SQLQuery1.Close;
           FormEnd.SQLQuery1.SQL.Clear;
