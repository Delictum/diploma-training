...
begin
  if (EditOboznach.Text = '') or (EditOboznach.Text = 'Обозначение') then
    begin
      ShowMessage('Введите обозначение.');
      exit;
    end;
  ...
  EditFoundMaxID.Text := VarToStr(SQLQuery3.FieldValues['Designation']);
    if EditFoundMaxID.Text = EditOboznach.Text then
      begin
        ShowMessage('Такой чертеж уже существует.');
        Exit;
      end;
  EditFoundMaxID.Text := '';

  if (EditNaim.Text = '') or (EditNaim.Text = 'Наименование') then
    begin
  ...
  if (CBNaznach.Text = '') then
    begin
      ShowMessage('Выберете назначение.');
      exit;
    end;
  if (CBSreda.Text = '') then
    begin
      ShowMessage('Выберете среду.');
      exit;
    end;
...
begin
  if CBCreate.ItemIndex < 0 then
    begin
      ShowMessage('Выберите создание из списка.');
      exit;
    end;
  if EditIzmerenie.Text = '' then
    begin
      ShowMessage('Введите новый элемент.');
      exit;
    end;
...
if (EditV_Min.Text = 'V_Min') then
  begin
    ShowMessage('Сотрите надпись "V_Min".');
    exit;
  end;
if (EditV_Max.Text = 'V_Max') then
  begin
    ShowMessage('Сотрите надпись "V_Max" и введите величины.');
    exit;
  end;
if (EditV_Max.Text = '') then
  begin
    ShowMessage('Введите "V_Max"');
    exit;
  end;
if (EditSymbol.Text = 'Символ') then
  begin
    ShowMessage('Сотрите надпись "Символ" и введите величины.');
    exit;
  end;
if (EditSymbol.Text = '') then
  begin
    ShowMessage('Введите "Символ"');
    exit;
  end;
if (CBIzmerenie.Text = '') then
  begin
    ShowMessage('Выберите еденицу измерения.');
    exit;
  end;
...
begin
  ...
  FormPDF.Show;
  FormPDF.Caption := 'Изменение изображения';
  FormPDF.Button1.Caption := 'Изменить';
end;
...
if (EditSymbol.Text = '') then
  ...
  EditFoundMaxID.Text := VarToStr(SQLQuery3.FieldValues['Symbol']);
    if EditFoundMaxID.Text <> EditSymbol.Text then
      begin
        ShowMessage('Такая характеристика не существует.');
        Exit;
      end;
  EditFoundMaxID.Text := '';
...
begin
  if CBCreate.ItemIndex < 0 then
    begin
      ShowMessage('Выберите удаление из списка.');
      exit;
    end;
  if EditIzmerenie.Text = '' then
    begin
      ShowMessage('Введите удаляемый элемент.');
      exit;
    end;
  if CBCreate.ItemIndex = 0 then
    begin
      ...
      begin
        ShowMessage('Такое наименование не существует.');
        Exit;
      end;
      EditFoundMaxID.Text := '';
...
begin
  ...
  FormPDF.Show;
  FormPDF.Caption := 'Удаление изображения';
  FormPDF.Button1.Caption := 'Удалить';
end;
...
begin
  if (EditSymbol.Text = '') or (EditSymbol.Text = 'Символ') then
    begin
      ShowMessage('Введите новый символ.');
      exit;
    end;
  if (DBEditOboznach.Text = '') then
    begin
      ShowMessage('Введите изменяемый символ.');
      exit;
    end;
  ClientDataSet3.Active := false;
   SQLQuery3.Close;
  ...
  bufSymbol := VarToStr(SQLQuery3.FieldValues['Symbol']);
    if bufSymbol <> DBEditSymbol.Text then
      begin
        ShowMessage('Изменяемый символ не существует.');
        Exit;
      end;
  if (EditV_Min.Text = 'V_Min') then
    begin
      ShowMessage('Введите новый V_Min.');
      exit;
    end;
  if (DBEditV_Min.Text = '') then
    begin
      ShowMessage('Введите изменяемый V_Min.');
      exit;
    end;
  if (EditV_Max.Text = 'V_Max') then
    begin
      ShowMessage('Введите новый V_Max.');
      exit;
    end;
  if (DBEditV_Min.Text = '') then
    begin
      ShowMessage('Введите изменяемый V_Max.');
      exit;
    end;

  if (CBIzmerenie.Text = '') then
    begin
      ShowMessage('Выберите новую еденицу измерения.');
      exit;
    end;
  ...
  if EditFoundMaxID.Text = '' then
    begin
      ShowMessage('Характеристика не найдена.');
      exit;
    end;
...
begin
  if CBCreate.ItemIndex < 0 then
    begin
      ShowMessage('Выберите редактирование из списка.');
      exit;
    end;
  if EEditIzmerenie.Text = '' then
    begin
      ShowMessage('Введите редактируемый элемент.');
      exit;
    end;
  if EditIzmerenie.Text = '' then
    begin
      ShowMessage('Введите измененный элемент.');
      exit;
    end;
...
begin
  if usCaptionC = false then
    begin
      BtnEdHandCharac.Caption := 'Обычное редактирование';
      usCaptionC := true;
    ...
    begin
      BtnEdHandCharac.Caption := 'Редактировать вручную';
      usCaptionC := false;
...
begin
  if usCaption = false then
    begin
      BtnEdHandZapis.Caption := 'Обычное редактирование';
      usCaption := true;
    ...
    begin
      BtnEdHandZapis.Caption := 'Редактировать вручную';
      usCaption := false;
...
begin
  if (EditOboznach.Text = '') or (EditOboznach.Text = 'Обозначение') then
    begin
      ShowMessage('Введите новое обозначение.');
      exit;
    end;
  if (DBEditOboznach.Text = '') then
    begin
      ShowMessage('Введите изменяемое обозначение.');
      exit;
    end;
  ClientDataSet3.Active := false;
   SQLQuery3.Close;
  ...
  bufOboznach := VarToStr(SQLQuery3.FieldValues['Designation']);
    if bufOboznach <> DBEditOboznach.Text then
      begin
        ShowMessage('Изменяемое обозначение не существует.');
        Exit;
      end;
  ClientDataSet3.Active := false;
   SQLQuery3.Close;
  ...
  if (bufOboznach = EditOboznach.Text) and (EditOboznach.Text <> DBEditOboznach.Text) then
    begin
      ShowMessage('Такое обозначение уже существует.');
      Exit;
    end;

  if (EditNaim.Text = '') or (EditNaim.Text = 'Наименование') then
    begin
      ShowMessage('Введите новое наименование.');
      exit;
    end;
  if (DBEditName.Text = '') then
    begin
      ShowMessage('Введите изменяемое наименование.');
      exit;
    end;
  ...
  if bufName <> DBEditName.Text then
    begin
      ShowMessage('Изменяемое наименование не существует.');
      Exit;
    end;

  if (CBNaznach.Text = '') then
    begin
      ShowMessage('Выберите новое назначение.');
      exit;
    end;

  if (CBSreda.Text = '') then
    begin
      ShowMessage('Выберите новую среду.');
      exit;
    end;
...
begin
  if (EditOboznach.Text = '') or (EditOboznach.Text = 'Обозначение') then
    begin
      ShowMessage('Введите удаляемое обозначение.');
      exit;
    end;
  ...
  EditFoundMaxID.Text := VarToStr(SQLQuery3.FieldValues['Designation']);
    if EditFoundMaxID.Text <> EditOboznach.Text then
      begin
        ShowMessage('Такой чертеж не существует.');
        Exit;
      end;
  EditFoundMaxID.Text := '';
