...
for i:=1 to StringGrid1.ColCount-1 do
 ...
 begin
  if StringGrid1.Cells[j,i]='' then
   ...
   i1:=inttostr(i);
   k:=0;
   memo2.Lines.Add('Ошибка В:'+j1+' столбце, '+i1+' строчке');
   j1:='0';
   i1:='0';
  end else k:=1;
 end;
...
Memo1.Clear;
memo2.Clear;
memo2.Lines.Add('Ошибки отсутствуют');
savegrid(StringGrid1,'test.txt');
memo1.Lines.Add('Очередь на заселение в общежитие:');
...
for i:=1 to StringGrid1.ColCount-1 do
 ...
 begin
  if StringGrid1.Cells[j,i]='' then
   ...
   i2:=inttostr(i);
   k:=0;
   memo2.Lines.Add('Ошибка В:'+j2+' столбце, '+i2+' строчке');
  end
  else
  begin
  k:=1;
