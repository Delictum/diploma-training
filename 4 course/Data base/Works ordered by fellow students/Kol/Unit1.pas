...
begin
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('SELECT id FROM productgroup WHERE name = "' + DBLookupComboBox1.Text + '"');
    SQLQuery1.Open;
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('SELECT MAX(id) AS ID FROM calories');
    SQLQuery1.Open;
    ClientDataSet1.Active := true;
    ...
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add
      ('INSERT INTO calories(id, groupsId, name, '
      + 'calorie) VALUES (');
    ...
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add
      ('SELECT calories.id, productGroup.Name, calories.Name, Calorie FROM productGroup, calories WHERE productGroup.Id = calories.GroupsId');
    SQLQuery1.Open;
    ClientDataSet1.Active := true;
...
begin
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('SELECT id FROM productgroup WHERE name = "' +
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('UPDATE calories Set groupsId = ' +
      bufName + ', name = "' + Edit1.Text +
      '", calorie = ' + Edit2.Text + ' WHERE id = ' + bufId);
    SQLQuery1.ExecSQL;
...
begin
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('DELETE FROM calories WHERE id = ' + bufId);
    SQLQuery1.ExecSQL;
...
begin
    DBGrid1.DataSource.DataSet.Locate('Name_1', VarArrayOf([edit5.Text]),
    [loCaseInsensitive, loPartialKey]);
end;

procedure TForm1.Button5Click(Sender: TObject);
begin
    if VarType(SQLQuery1.Lookup('Name', Edit4.Text, 'Calorie')) <> VarNull then
      ShowMessage('Калории: ' +
...
begin
    ...
    sheet.cells[1, 2] := 'Продукт';
    sheet.cells[1, 3] := 'Калории';
...
begin
  ...
  exSh := exBook.Worksheets[1];
    ...
    index := 2;
    exSh.cells[2, 1] := 'Группа';
    exSh.cells[2, 2] := 'Продукт';
    exSh.cells[2, 3] := 'Калории';
...
begin
    ...
    SQLQuery1.SQL.Add
      ('SELECT calories.id, productGroup.Name, calories.Name, Calorie FROM productGroup, calories WHERE productGroup.Id = calories.GroupsId ' +
      'AND productGroup.Name LIKE "%' + edit6.Text + '%"');
    SQLQuery1.Open;
    ClientDataSet1.Active := true;
end;
