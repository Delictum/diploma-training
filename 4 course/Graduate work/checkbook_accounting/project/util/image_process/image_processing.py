...
from project.util.image_process.AbbyyOnlineSdk import AbbyyOnlineSdk, ProcessingSettings

processor = None


def gaussian_process_img(img, sigma):
    """Return PIL.Image

    Remove noise from the image (smoothing from gaussian filter)
    """
    a = np.asarray(img)
    blurred = gaussian_filter(a, sigma=sigma)
...
def start_abbyy(target_file, source_file):
    global processor
